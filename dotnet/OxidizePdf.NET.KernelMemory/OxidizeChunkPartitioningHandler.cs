...
/// <c>ExtractedContent</c> artifact written by KM's <c>extract_text</c> handler
...
public sealed class OxidizeChunkPartitioningHandler : IPipelineStepHandler
{
    ...
    /// </summary>
    /// <param name="stepName">Pipeline step name, typically <c>split_text_in_partitions</c>.</param>
        ...
        DataPipeline pipeline, CancellationToken cancellationToken = default)
    {
        foreach (DataPipeline.FileDetails uploadedFile in pipeline.Files)
        {
            ...
            foreach (DataPipeline.GeneratedFileDetails generated in uploadedFile.GeneratedFiles.Values)
            {
                ...
                FileContentDto? content = JsonSerializer.Deserialize<FileContentDto>(raw.ToArray());
